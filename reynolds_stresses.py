...
PBLS = ['YSU']
CLS = ['1.0','lvl_2','lvl_4','lvl_8'] #,'vonkarman_0.2','vonkarman_0.6','q_4.0']
		...
		for TM in TMS:
			for PBL in PBLS:
				for CL in CLS:
					...
					for csv_file in csv_files[16:18]:	#6:8 kinda ok
						print(csv_file)
...
# 	# 					# Plot Reynolds stresses.
						print('l=',l)
						print('l+3 =',l+3)
						axess[l].plot(Rey_uruth, z_5000, color=colors[c], linewidth=2, label = CL)
						axess[l+1].plot(Rey_uruz, z_5000, color=colors[c], linewidth=2, label = CL)
						axess[l+2].plot(Rey_uthuz, z_5000, color=colors[c], linewidth=2, label = CL)
